...
import com.fanwe.p2p.constant.Constant.LoanType;
import com.fanwe.p2p.constant.Constant.LoanTypeString;
import com.fanwe.p2p.constant.Constant.RiskRank;
import com.fanwe.p2p.constant.Constant.RiskRankString;
...
public class DealsActItemModel implements Serializable
{
	...
	public void setDeal_status(String deal_status)
	{
		...
		if (!TextUtils.isEmpty(deal_status))
		{
			...
			{
				this.deal_status_format_string = DealStatusString.FAIL;
			} else if (deal_status.equals(DealStatus.FULL))
			{
				this.deal_status_format_string = DealStatusString.FULL;
			} else if (deal_status.equals(DealStatus.LOANING)) // 借款中
			{
				this.deal_status_format_string = DealStatusString.LOANING;
				this.isLoaning = true;
			} else if (deal_status.equals(DealStatus.REPAYMENTED))
			{
				this.deal_status_format_string = DealStatusString.REPAYMENTED;
			} else if (deal_status.equals(DealStatus.REPAYMENTING))
			{
				this.deal_status_format_string = DealStatusString.REPAYMENTING;
			} else if (deal_status.equals(DealStatus.WAIT))
			{
				this.deal_status_format_string = DealStatusString.WAIT;
			} else
			{
	...
	public void setLoantype(String loantype)
	{
		...
		if (loantype != null)
		{
			...
			{
				this.loantype_format = LoanTypeString.ZERO;
			} else if (loantype.equals(LoanType.ONE))
			{
				this.loantype_format = LoanTypeString.ONE;
			} else if (loantype.equals(LoanType.TWO))
			{
				this.loantype_format = LoanTypeString.TWO;
			}
		}
	}
