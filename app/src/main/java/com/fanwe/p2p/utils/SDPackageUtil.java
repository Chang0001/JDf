...
public class SDPackageUtil
{
	...
	public static void startAPP(String appPackageName)
	{
		try
		{
			Intent intent = App.getApplication().getPackageManager().getLaunchIntentForPackage(appPackageName);
	...
	{
		if (!App.getApplication().getmRuntimeConfig().isMainActivityStarted())
		{
			startAPP(App.getApplication().getPackageName());
		}
	}
