...
import com.fanwe.p2p.adapter.PublishedBorrowAdapter;
...
import com.fanwe.p2p.model.act.Uc_BorrowedActModel;
...
public class PublishedBorrowActivity extends BaseActivity
{
	...
	{
		mAdapter = new PublishedBorrowAdapter(mListModel, this);
		...
		mLsvDeals.setOnItemClickListener(new OnItemClickListener()
		{
			...
			public void onItemClick(AdapterView<?> parent, View view, int position, long id)
			{
				...
				if (SDCollectionUtil.isListHasData(mListModel) && id < mListModel.size())
				{
					...
					{
						Intent intent = new Intent(getApplicationContext(), ProjectDetailWebviewActivity.class);
	...
	private void requestData(final boolean isLoadMore)
	{
		...
		if (user != null)
		{
			...
			SDAsyncHttpResponseHandler handler = new SDAsyncHttpResponseHandler()
			{
				...
				{
					Uc_BorrowedActModel actModel = JSON.parseObject(content, Uc_BorrowedActModel.class);
					return actModel;
				}
				...
				{
					Uc_BorrowedActModel actModel = (Uc_BorrowedActModel) result;
